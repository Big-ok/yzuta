...
$(document).ready(function(){
    ...
    (function() {
        ...
        $block.on('scroll', function() {
            var position = $block[0].scrollHeight - $block[0].offsetHeight - $(this).scrollTop();
            if(position > 10) {
        ...
        $.each(sliderWrap, function(index, value){
                    ...
                    if(sliderPagination.length>0){
                        sliderOptions.pagination = {
                            ...
                            renderFraction: function (currentClass, totalClass) {
                                return '<span class="slider-fraction__item ' + currentClass + '"></span>' +
                                    '<span class="slider-fraction__item slider-fraction__item--divider">.</span>' +
                                    '<span class="slider-fraction__item ' + totalClass + '"></span>';
                            },
            ...
            if ($timeline.length>0) {
                ...
                var timelineParent = $timeline.parents('.js-slider-wrap'),
                    sliderInstance = timelineParent.find('.js-slider-container')[0].swiper,
                    ...
                    currentOffset = window.matchMedia('(max-width: 1199px)').matches ? 0 : 1,
                    pointsObj = Object.create(null);

                $points.each(function(index, point) {
                    var $point = $(point),
                       pointOffset = $slides.filter('[data-step="' + $point.attr('data-slide-year') + '"]').index();
                    if (index === 0) {
                        $point.css('left', '0');
                ...
                function moveBar(offset) {
                    var ratio = (offset * stepPercent) / 100,
                        template = '-ms-transform: translate3d(0px, 0px, 0px) scaleX({{length}}) scaleY(1); ' +
    ...
    if(mapContainer.length){
        ...
        function init(){
            ...
            var placemark = new ymaps.Placemark([55.764598, 37.612788], {
                balloonContent: "",
                iconContent: ""
            }, {
    ...
    (function() {
        ...
        var distance = $items.toArray().slice(0, $active.index()).reduce(function (total, item) {
            return total += $(item).outerWidth();
        }, 0) + $active.outerWidth() / 2;
